...
if gcloud compute instances describe instance-1 --zone "$ZONE" >/dev/null 2>&1; then
  echo "${COLOR_YELLOW}${BOLD}instance-1 exists. Deleting first...${COLOR_RESET}"
  gcloud compute instances delete instance-1 --zone "$ZONE" --quiet
fi

gcloud compute instances create instance-1 --project="$PROJECT_ID" --zone="$ZONE" \
...
gcloud compute instances ops-agents policies create "goog-ops-agent-v2-x86-template-1-4-0-$ZONE" \
  --project="$PROJECT_ID" --zone="$ZONE" --file=config.yaml || true

gcloud compute resource-policies create snapshot-schedule default-schedule-1 \
  --project="$PROJECT_ID" --region="$REGION" --max-retention-days=14 \
  --on-source-disk-delete=keep-auto-snapshots --daily-schedule --start-time=08:00 || true

gcloud compute disks add-resource-policies instance-1 --project="$PROJECT_ID" --zone="$ZONE" \
